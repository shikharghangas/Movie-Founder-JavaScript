    ...
    axios.get('http://www.omdbapi.com?apikey=5acafce3&s=' + searchText)
        .then((response) => {
            ...
            if (movies) {
                ...
                $('#movies').html('<p class="text-center">No movies found.</p>');
            }
        })
        .catch((err) => {
            console.log(err);
            $('#movies').html('<p class="text-center">Something went wrong. Please try again later.</p>');
        });
}
...
function getMovie() {
    ...
    axios.get('http://www.omdbapi.com?apikey=5acafce3&i=' + movieId)
        ...
        .catch((err) => {
            console.log(err);
            $('#movie').html('<p class="text-center">Something went wrong. Please try again later.</p>');
        });
}
